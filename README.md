...
> **目前狀態（2026-08-23）**：本機品質門檻、PostgreSQL 16 migration/runtime、96 個角色安全檢查與瀏覽器 E2E 已通過。業務階層為 `GM` → `MARKETING_MANAGER`／`SALES_MANAGER` → `SALES`；`ORDER_ADMIN` 是跨市場的訂單管理／支援角色。這代表「可驗證」而非「已完成正式上線」；正式切換、MFA/SSO、外部告警、離機備份與集中式觀測仍在改善計畫中。
...
* **訂單管理員範圍**：`ORDER_ADMIN` 為跨市場支援單位，可讀取／建立／更新全市場商機；不可管理使用者、決策報表或安全稽核。
...
如需在**可重設的** demo DB 建立人員組織結構（角色、區域、主管階層）與標準銷售管線，可於啟動前自行提供 demo 密碼；seed 會**清除該資料庫所有既有資料**（須設定 `DEMO_SEED_CONFIRM=1` 確認），且不會建立任何示範業務資料（客戶、商機、工單等）：

```powershell
$env:DEMO_SEED_PASSWORD = Read-Host "Demo password" -MaskInput
$env:DEMO_SEED_CONFIRM = "1"   # seed 會清除既有資料，需明確確認
npm run db:seed
```
